...
default_cfg = os.path.dirname(os.path.realpath(__file__)).replace(
    "install/iwr6843aop_pub/lib/python3.8/site-packages/iwr6843aop_pub", "/src/iwr6843aop_pub-main/cfg_files") + "/" + "yang2d.cfg"
...
class TI:
    ...
    def _configure_radar(self, config):
        for i in config:
            ...
            if idx != -1:
                ...
                print("Found frameCfg, milliseconds per frame is ",
                      i.split()[5])
            time.sleep(0.01)
    ...
    def _parse_header_data(self, byte_buffer, idx):
        ...
        """
        magic, idx = self._unpack(
            byte_buffer, idx, order='>', items=1, form='Q')
    ...
    def _parse_header_tlv(self, byte_buffer, idx):
        ...
        """
        (tlv_type, tlv_length), idx = self._unpack(
            byte_buffer, idx, items=2, form='I')
    ...
    def _process_azimut_heat_map(self, byte_buffer):
        ...
        azimuth_map = np.zeros(
            (self.num_virtual_ant, self.config_params['numRangeBins'], 2), dtype=np.int16)
        ...
        for bin_idx in range(self.config_params['numRangeBins']):
            ...
            for ant in range(self.num_virtual_ant):
                azimuth_map[ant][bin_idx][:], idx = self._parse_msg_azimut_static_heat_map(
                    byte_buffer, idx)
        return azimuth_map
    ...
    def _process_detected_points(self, byte_buffer):
        ...
        (tlv_type, tlv_length), idx = self._parse_header_tlv(byte_buffer, idx)
        for i in range(num_points):
            (snr, noise), idx = self._parse_msg_detected_points_side_info(
                byte_buffer, idx)
            data[i][4] = snr
            data[i][5] = noise
    ...
    def _unpack(byte_buffer, idx, order='', items=1, form='I'):
        ...
        try:
            data = struct.unpack(order + str(items) + form,
                                 byte_buffer[idx:idx + (items * size[form])])
            if len(data) == 1:
                data = data[0]
...
class Detected_Points:
    ...
    # 'COM4',data_loc='COM3'):
    def data_stream_iterator(self, cli_loc=cli_port, data_loc=data_port):
...
class MinimalPublisher(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(PointCloud2, 'points2', 10)
        ...
        if not xyzdata == []:
            ...
            pcl_msg.fields = [PointField(name='x', offset=0, datatype=PointField.FLOAT32, count=1),
                              ...
                              PointField(name='z', offset=8,
                                         datatype=PointField.FLOAT32, count=1),
                              PointField(name='v', offset=12,
                                         datatype=PointField.FLOAT32, count=1),
                              PointField(name='snr', offset=16,
                                         datatype=PointField.INT16, count=1),
                              PointField(name='noise', offset=18, datatype=PointField.INT16, count=1)]
            # cloud_msg.is_bigendian = False # assumption
            # size of 1 point (float32 * dimensions (3 when xyz))
            pcl_msg.point_step = cloud_arr.dtype.itemsize*cloud_arr.shape[1]
            pcl_msg.row_step = pcl_msg.point_step * \
                cloud_arr.shape[0]  # only 1 row because unordered
...
class iwr6843_interface(object):
    def __init__(self):
        ...
        self.stream = detected_points.data_stream_iterator(
            cli_port, data_port)  # 'COM4','COM3',1000)
